...
fn part_one(input: &str) -> usize {
    ...
    let result: usize = energized.iter().map(|line| line.iter().sum::<usize>()).sum();
...
fn track_beam(
    ...
    {
        match seen_points.insert(Beam { start_point: current_point.clone(), direction: current_direction.clone() }) {
            true => (),
            false => break
        }
        ...
        match current_char {
            '.' => match current_direction {
                ...
                Direction::Left => current_point = [current_point[0].wrapping_sub(1), current_point[1]],
                Direction::Up => current_point = [current_point[0], current_point[1].wrapping_sub(1)],
