...
fn part_one(input: &str) -> usize {
    ...
    for galaxy in galaxies.iter_mut() {
        ...
        galaxy[0] = galaxy[0] + (*x_offset * 999999);
        galaxy[1] = galaxy[1] + (*y_offset * 999999);
    }
...
fn euclidean_distance(point_a: &[usize; 2], point_b: &[usize; 2]) -> usize {
    let distance = (point_b[0] as isize - point_a[0] as isize).abs()
        + (point_b[1] as isize - point_a[1] as isize).abs();
