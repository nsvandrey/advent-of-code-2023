...
fn count_items(hand: &Vec<char>) -> HashMap<char, usize> {
    ...
    if card_map.contains_key(&'J') && card_map.len() > 1 {
        let joker_count = card_map.get(&'J').unwrap().clone();
        let mut max_count = 0;
        let mut max_card: &char = &'J';

        for (key, value) in card_map.iter() {
            if key == &'J' {
                continue;
            }
            ...
            if value > &max_count {
                ...
                if CARD_VALUES.iter().position(|&val| val == *key)
                    > CARD_VALUES.iter().position(|&val| val == *max_card)
                {
                    max_card = key;
                }
            }
        ...
        let count = card_map.entry(*max_card).or_insert(0);
        *count += joker_count;
